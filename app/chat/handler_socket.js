        ...
        if (typeof(id_2_find) == 'undefined') {
          Users.findOne({
            ...
            if (err) {
              ...
              user_exist.users_chat.push({
                id: id_2,
                username: user_2.username,
                avatar: user_2.avatar,
                avatar_small: user_2.avatar_small
              });
  ...
  chat.on('connection', function(socket) {
    ...
    socket.on('Typing', function(data) {
      ...
      if (user_sockets[user_recei]) {
        user_sockets[user_recei].emit('Typing', {
          user_send: user_send
        });
      };
    })
    ...
    socket.on('stopTyping', function(data) {
      ...
      var user_recei = data.user_recei;
      if (user_sockets[user_recei]) {
        user_sockets[user_recei].emit('stopTyping', {
          user_send: user_send
        });
      };
    })
    ...
    socket.on('Read messages', function(data) {
      ...
      Users.findOne({
        ...
        if (err) {
          ...
          if (user_exist && user_exist.unread_msg > 0) {
            ...
            user_exist.save(function(err) {});
            socket.emit('unread_msg', {
              unread_msg: user_exist.unread_msg
            });
      ...
      if (user_sockets[user_sent]) {
        user_sockets[user_sent].emit('Read messages', {
          user_read: user_read,
          user_sent: user_sent
        });
      }
    ...
    socket.on('Send message', function(data) {
      ...
      if (user_sockets[user_recei]) {
        Users.findOne({
          ...
          Users.findOne({
            _id: user_recei
          }, function(err, user_recei_exist) {
            user_sockets[user_recei].emit('unread_msg', {
              unread_msg: user_recei_exist.unread_msg
            })
          })
